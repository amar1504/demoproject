...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    ...
    <div class="radio">
    <label><input name="status" type="radio" value="1" {{ (isset($configuration) && 1 == $configuration->status) ? 'checked' : '' }} required=""> Active</label>
</div>
<div class="radio">
    <label><input name="status" type="radio" value="0" {{ (isset($configuration) && 0 == $configuration->status) ? 'checked' : '' }} > Inactive</label>
</div>
...
<div class="form-group">
    ...
    <a href="{{ url('/admin/configuration') }}" title="Back"><button type="button" class="btn btn-warning"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</button></a>
</div>

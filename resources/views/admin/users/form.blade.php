...
<div class="form-group {{ $errors->has('password') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="password" type="text" id="password" value="{{old('password')}}" data-parsley-length="[8, 12]" required="">
    {!! $errors->first('password', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group {{ $errors->has('confirmpassword') ? 'has-error' : ''}}">
    <label for="confirm password" class="control-label">{{ 'Confirm Password' }}</label>
    <input class="form-control" name="confirmpassword" type="text" id="confirmpassword" value="{{old('confirmpassword')}}" data-parsley-equalto="#password"  required="">

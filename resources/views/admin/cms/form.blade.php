...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    ...
    <div class="radio">
    <label><input name="status" type="radio" value="1" {{(old('status') == '1') ? 'checked' : ''}} {{ (isset($cm) && 1 == $cm->status) ? 'checked' : '' }} required=""> Active</label>
</div>
<div class="radio">
    <label><input name="status" type="radio" value="0" {{(old('status') == '0') ? 'checked' : ''}} {{ (isset($cm) && 0 == $cm->status) ? 'checked' : '' }} > Inactive</label>
</div>
